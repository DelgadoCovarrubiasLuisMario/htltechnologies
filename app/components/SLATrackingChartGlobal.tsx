...
import { getAllSLAs } from '@/app/utils/slaStorage';

type FilterType = 'vencidos' | 'terminados' | 'activos';

export default function SLATrackingChartGlobal() {
  ...
  const generateChart = () => {
    ...
    ctx.fillText('Seguimiento y medición del cumplimiento del SLA (Global)', width / 2, 40);
  ...
  const handleDownload = () => {
    if (imageUrl) {
      ...
      link.download = `Seguimiento_SLA_Global_${Date.now()}.png`;
      link.href = imageUrl;
      link.click();
    }
  };
      ...
      <div style={{ 
        ...
        {imageUrl && (
          <>
            <img 
              src={imageUrl} 
              alt="Gráfica de seguimiento SLA Global"
              style={{
                maxWidth: '500px',
                width: '100%',
